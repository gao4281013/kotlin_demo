...
import com.example.administrator.mykotlin_project.utils.NetworkUtils
...
class CacheInterceptor(context: Context):Interceptor{
    ...
    override fun intercept(chain: Interceptor.Chain?): Response? {
        ...
        if (NetworkUtils.isNetConnected(context)){
            ...
            return response?.newBuilder()?.removeHeader("Pragma")?.removeHeader("Cache-Control")?.header("Cache-Control","public, only-if-cached,max-state="+maxStale)?.build()
        }
    }
}